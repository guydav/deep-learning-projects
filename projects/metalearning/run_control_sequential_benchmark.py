...
if __name__ == '__main__':
    ...
    wandb.init(entity='meta-learning-scaling', project=args.wandb_project, name=f'{args.name}-{dataset_random_seed}')

    description = args.description
    if len(description) > 0:
        description += '\n'

    description += f'coreset size: {train_coreset_size}, dataset random seed: {dataset_random_seed}, query order: {list(query_order)}, threshold all queries: {threshold_all_queries}'
